				...
				for (Element element : listings) {
					...
					for (Element elem : strVehicleInfo) {
						if (elem.text().contains("miles")) {
							//System.out.println("Miles : " + elem.text().substring(0, elem.text().indexOf("miles"))
								//	.replace("Mileage:", "").replace(",", "").trim());
							details.setMilesRun(Integer.parseInt(elem.text().substring(0, elem.text().indexOf("miles")).replace("Mileage:", "")
									.replace(",", "").trim()));
						}
					}
					...
					for (Element elem : strPrices) {
						if (elem.hasText())
							//System.out.println("Price : " + elem.text().replace("$", "").replace(",", "").trim());
						details.setSalePrice(Integer.parseInt(elem.text().replace("$", "").replace(",", "").trim()));
					}
