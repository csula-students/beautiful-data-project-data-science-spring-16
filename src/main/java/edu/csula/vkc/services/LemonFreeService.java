	...
	public JsonNode getMakes() {
		...
		try {
			responseMake = Unirest
					.get("http://api.lemonfree.com/v2/makes/?format=json&key="+getKey()).asJson()
					.getBody();
	...
	public JsonNode getModelsByMakeID(long id) {
		...
		try {
			responseModelsByMakeID = Unirest
					.get("http://api.lemonfree.com/v2/models/?make_id="+id+"&format=json&key="+getKey())
					.asJson().getBody();
		} catch (UnirestException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public JsonNode getListingsbyMakeAndModel(String makeName, String modelName) {
		...
		try {
			...
			responseListingsbyMakeAndModel = Unirest
					.get("http://api.lemonfree.com/v2/listings/?make="+makeName+"&model="+modelName+"&format=json&key=07859aa5804ac8c1425bdb703ca961ed")
					.asJson().getBody();
		} catch (UnirestException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
